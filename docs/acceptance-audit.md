...
This audit maps the original offline-only Go port acceptance criteria to
current repository evidence. It is intentionally scoped to local-file offline
access. FTP, RCON, and live-server mutation validation are outside the accepted
scope.
...
| `main` is the pushed working branch. | Met | `git ls-remote --heads origin main` and local `git rev-parse HEAD` both reported `5cdff60cab557199c9e85cdee2d33c45ee790872` before this audit update. |
...
| Public CI verifies the repository without private data. | Met | GitHub Actions `verify` workflow run `28346595359` passed on `main` after commit `5cdff60`. |
