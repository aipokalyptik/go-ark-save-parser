...
- `examples/base_api/ex_01_export_base_from_save.py` is represented by
  `examples/base_export_from_save`, which writes base metadata, copied raw
  structure rows, and structure location JSON files to an explicit output
  directory. Inventory item expansion, binary import, and live-server
...
- `examples/dino_api/ex_13_extract_and_reinsert_dino.py` is partially
  ...
  `arkmutation.ImportDinoBinary`, which can reinsert copied direct-save dino,
