...
| P2-API-006 | `[~]` | Finish full structure/base edge behavior. | Implemented coverage includes base/structure binary export-import round trips, typed structure health summaries, owner-location skip counters, an exact full-parse owner/location API, shared synthetic-tested heatmap boundary semantics for in-range/out-of-range/invalid coordinate values, and skipped-coordinate reporting in heatmap summaries, CLI output, and examples. Remaining upstream/private `structure_heatmap` numeric oracle parity is blocked by upstream private-save cell indexing; base customization and live-server acceptance are out of local verification scope. |
