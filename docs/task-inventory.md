...
| P3-API-002 | `[~]` | Replace remaining Python-shaped compatibility helpers where typed Go surfaces now exist. | `parse_all`, `object_classes`, `object_summary`, `class_property_summary`, `class_lookup`, `property_filter`, and `property_positions` now use typed `arkapi.GeneralAPI` helpers; player/tribe examples now use `arkapi.NewPlayerFromPath`; `player_inventories` now uses `arkapi.PlayerInventorySummaryFromPath`; remaining generic inspection and mutation examples keep low-level access where that is the feature. |
