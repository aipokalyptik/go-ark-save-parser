...
| P4-VERIFY-004 | `[x]` | Go-only provided-data E2E smoke test is available. | `make e2e-test` exercises selected read-only APIs, object class lists and object summaries, class/property lookup and class property summaries, stackable owned/count, dino/equipment domain JSON API/CLI export, bounded structure/base API checks including structure heatmap JSON, CLI commands, local tribute/cluster handling, and examples against configured private/provided data and skips without env vars. |
