...
| P2-API-008 | `[~]` | Finish richer local cluster item/dino domain models. | Uploaded item type has typed constants/accessors, enum filters, version/parse helpers, crafter helpers, item aggregate summaries, explicit uploaded-dino parse status counts, dino parse/component aggregate summaries, and JSON parse/version/component status fields; add deeper item/dino fields only when chosen local-file fixtures expose them. |
