...
| P3-FIX-001 | `[~]` | Replace duplicated synthetic fixture builders with internal helpers. | `internal/testfixtures` centralizes many shared fixtures, including player pawn, inventory, structure, equipment, stackable, basic dino save-object builders, binary `CustomItemDatas` writers, ID-table Vector struct property writers, base linked-structure object-reference array writers, player/tribe relation directory fixtures, and ID-table game-object bytes with custom object-name payloads; `internal/propertyfixtures` centralizes parsed `CustomItemDatas` cryopod/custom-data builders; specialized status and malformed parser fixtures remain. |
