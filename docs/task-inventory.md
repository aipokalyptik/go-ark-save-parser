...
| P2-API-007 | `[blocked]` | Finish full equipment edge behavior. | Implemented coverage includes ranking candidate selection, upstream-style effective internal average stats, inventory-state summaries, equipment item quantity/rating averages, upstream-list classification guards, upstream family/slot default stat tables, cursed shield defaults, inferable cursed weapon defaults, Tek sword durability, generic `CustomItemDatas` metadata, and typed unsupported cryopod saddle fault handling. Remaining exact private ranking comparison would require expanding the Python oracle suite; legacy/modded cryopod saddle payloads, cosmetics, and future default-table mismatches need concrete fixtures or failures. |
