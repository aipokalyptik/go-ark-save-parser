...
| P3-CLI-001 | `[x]` | Implement offline CLI commands. | `inspect`, `parse`, `structure-health`, `structure-owner-count`, `structure-owners`, `structure-owner-locations`, `base-components`, `dinos`, `equipment-summary`, `players`, `tribes`, `cluster`, `tribute`, JSON export commands, and experimental mutate commands exist. |
