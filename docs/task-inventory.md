...
| P2-API-008 | `[~]` | Finish richer local cluster item/dino domain models. | Uploaded item type has typed constants/accessors, enum filters, version/parse helpers, crafter helpers, and embedded dino component class summaries; add deeper item/dino fields only when chosen local-file fixtures expose them. |
| P2-API-009 | `[~]` | Finish remaining Player/Tribe edge behavior. | Typed player pawn inventory indexing, upstream-style inventory item counting, save-backed inventory summaries, and path-level player inventory summaries now live in `arkapi`; remaining upstream edge cases beyond parsed local archives, save objects, and embedded `GameModeCustomBytes` remain. |
...
| P3-PKG-002 | `[ ]` | Further split large domain models under `arkobject` or subpackages. | Broad dino, structure, equipment, stackable, player, tribe, inventory, and cluster model split remains. |
...
| P3-API-003 | `[ ]` | Add remaining full typed API layers and model-specific JSON exports. | Stackable count/owner-filter, structure tribe-ownership, dino wild-tamable, and shared heatmap summaries now have typed API helpers; full dino, full structure, equipment, base, cluster, and player/tribe parity remain incremental. |
