...
| P3-PKG-002 | `[~]` | Further split large domain models under `arkobject` or subpackages. | Shared crafter metadata now lives in its own `arkobject` file; equipment item construction, stat calculation, default stat tables, and property coercion helpers are split into focused files with a package-shape regression test. Broader dino, structure, stackable, player, tribe, inventory, and cluster subpackage splits remain. |
