...
| P3-API-002 | `[x]` | Replace remaining Python-shaped compatibility helpers where typed Go surfaces now exist. | `map_summary`, `parse_all`, `object_classes`, `object_summary`, `class_property_summary`, `class_lookup`, `property_filter`, and `property_positions` use typed `arkapi.GeneralAPI` or JSON helpers, with shared `arkapi.NewGeneralFromPath` open/close handling for GeneralAPI examples; player/tribe examples use `arkapi.NewPlayerFromPath`; `player_all`, `player_list`, `tribe_list`, `player_tribe_links`, and `player_and_tribe_data` use typed roster/relation/aggregate summaries; `local_profiles` uses typed local-file and tribe-player link summaries; `local_tribute`, `tribute_json`, `cluster_json`, and `cluster_typed` use typed path helpers; player inventory examples use `arkapi.PlayerInventorySummaryFromPath` and `arkapi.PlayerInventoryLookupFromPath`; equipment, dino, structure, stackable, base, heatmap, owner-location, and all-domain JSON examples now use typed `arkapi` helpers. Remaining low-level examples are generic inspection, explicit binary export, or mutation-copy workflows where direct save/file handling is the feature. |
| P3-API-003 | `[ ]` | Add remaining full typed API layers and model-specific JSON exports. | Stackable count/owner-filter path summaries, including fault-preserving aggregate CLI helper, structure tribe-ownership/owner/health path summaries plus aggregate CLI usage, structure heatmap summary, dino population/wild-tamable/baby path summaries and aggregate CLI usage, equipment inventory-state/path summary plus aggregate CLI usage, equipment owned-by path summary plus CLI usage, equipment saddle path summary including cryopod saddles plus CLI usage, equipment path summary including cryopod saddles, player roster/player-all/tribe roster/player-tribe relation/player-and-tribe-data path summaries plus CLI/example usage, base aggregate/component path summaries plus base-components CLI/example usage, local cluster file/directory summary, path-level dino/structure heatmap summaries, and shared heatmap summaries now have typed API helpers; full dino, full structure, equipment, base, cluster, and deeper player/tribe parity remain incremental. |
...
| P3-FIX-001 | `[~]` | Replace duplicated synthetic fixture builders with internal helpers. | `internal/testfixtures` centralizes many shared fixtures, including player pawn, inventory, structure, equipment, stackable, basic dino save-object builders, dino stats/status object byte fixtures, binary `CustomItemDatas` writers, ID-table Vector struct property writers, base linked-structure object-reference array writers, player/tribe relation directory fixtures, and ID-table game-object bytes with custom object-name payloads; `internal/propertyfixtures` centralizes parsed `CustomItemDatas` cryopod/custom-data builders; specialized status and malformed parser fixtures remain. |
