...
| SCOPE-007 | `[x]` | Do not expand the Python codebase or oracle suite beyond selected-feature parity needs. | Existing oracle artifacts remain reference evidence. Narrow throwaway-style oracle probes are allowed for concrete blocked parity questions; broad upstream/Python oracle improvement remains out of scope. |
...
| P2-PROP-004 | `[~]` | Port legacy property/object parsing where a runnable offline oracle path exists. | Go now parses synthetic legacy archive version/object/class metadata into typed `LegacyArchiveError` faults for local profile and cluster directory flows while preserving `ErrLegacyArchiveUnsupported`. Full legacy object/property parsing remains blocked until a committed fixture or private runnable oracle case proves exact behavior. |
...
| P2-API-005 | `[blocked]` | Finish full dino edge behavior. | Implemented coverage includes typed pedigree tree helpers, domain JSON pedigree trees, typed malformed cryopod dino payload faults, typed unsupported cryopod saddle payload faults with an `errors.Is` sentinel, and fixture-backed cryopod item transform fallback for cryopodded dino locations. Remaining legacy/modded cryopod parsing and full private pedigree comparison are blocked by malformed/unsupported cryopod payload paths or missing focused fixtures. |
| P2-API-006 | `[~]` | Finish full structure/base edge behavior. | Implemented coverage includes base/structure binary export-import round trips, typed structure health summaries, owner-location skip counters, an exact full-parse owner/location API, computed offline demolishable-structure reports using `LastInAllyRangeTimeSerialized` with `LastEnterStasisTime` fallback, shared synthetic-tested heatmap boundary semantics for in-range/out-of-range/invalid coordinate values, and skipped-coordinate reporting in heatmap summaries, CLI output, and examples. Remaining upstream/private `structure_heatmap` numeric oracle parity is blocked by upstream private-save cell indexing; base customization and live-server acceptance are out of local verification scope. |
| P2-API-007 | `[blocked]` | Finish full equipment edge behavior. | Implemented coverage includes ranking candidate selection, upstream-style effective internal average stats, inventory-state summaries, equipment item quantity/rating averages, upstream-list classification guards, upstream family/slot default stat tables, cursed shield defaults, inferable cursed weapon defaults, Tek sword durability, generic `CustomItemDatas` metadata, typed unsupported cryopod saddle fault handling, and an oracle-probe-backed guard that preserves the upstream `equipment_rank` ignore token exactly. Remaining exact private ranking comparison is narrowed to a small ranked-row/stat edge; legacy/modded cryopod saddle payloads, cosmetics, and future default-table mismatches need concrete fixtures or failures. |
| P2-API-008 | `[blocked]` | Finish richer local cluster item/dino domain models. | Implemented coverage includes typed uploaded item/dino summaries, parse/version/component status fields, short names, embedded uploaded-dino identity and stat-level projections, JSON/CLI detail output, identity/stat aggregate counts, embedded dino level aggregates, uploaded-item quantity/rating averages, uploaded item/dino time-range aggregates, fault-preserving directory reads, directory fault reporting, and aggregate summaries. Deeper item/dino fields are fixture-gated until chosen local-file data exposes them. |
