...
- [x] Go-only provided-data E2E smoke tests cover stackable, stackable-owned,
      dino, and equipment command/API paths, including equipment-owned-by
      summaries, plus domain JSON export through both
      `arkapi.JSONAPI.ExportDomain` and the CLI `export-domain-json` path.
